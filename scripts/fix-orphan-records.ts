...
async function cleanupComments(userIds: Set<string>) {
  ...
  if (orphanComments.length > 0) {
    ...
    await prisma.characterComment.deleteMany({
      where: { id: { in: orphanComments.map((c) => c.id) } },
    });
...
async function cleanupFollows(userIds: Set<string>) {
...
async function cleanupDiaries(userIds: Set<string>) {
  ...
  if (orphanDiaries.length > 0) {
    ...
    await prisma.diaryEntry.deleteMany({
      where: { id: { in: orphanDiaries.map((d) => d.id) } },
    });
...
async function cleanupTrivia(userIds: Set<string>) {
  ...
  if (orphanTrivia.length > 0) {
    ...
    await prisma.triviaFact.deleteMany({
      where: { id: { in: orphanTrivia.map((t) => t.id) } },
    });
...
async function cleanupCollections(userIds: Set<string>) {
  ...
  if (orphanCollections.length > 0) {
    ...
    await prisma.quoteCollection.deleteMany({
      where: { id: { in: orphanCollections.map((c) => c.id) } },
    });
...
async function cleanupProgress(userIds: Set<string>) {
...
async function fixOrphanRecords() {
  ...
  try {
    ...
    await cleanupComments(userIds);
    await cleanupFollows(userIds);
    await cleanupDiaries(userIds);
    await cleanupTrivia(userIds);
    await cleanupCollections(userIds);
    await cleanupProgress(userIds);
